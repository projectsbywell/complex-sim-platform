...
def test_export_state_missing_lines(tmp_path):
    from simcore.export import _flatten_state, export_csv, export_json, export_parquet, export_hdf5
    ...
    assert len(rows3)==4
    # bodies
    rows4 = _flatten_state({"bodies":[{"x":1,"y":2,"vx":0,"vy":0,"mass":1,"radius":1}]})
    assert rows4[0]["x"]==1
    # positions
...
def test_particles_physics_edges():
    ...
    p._bodies[0].x = 50; p._bodies[0].y=50
    p._bodies[1].x = 50; p._bodies[1].y=50
...
def test_security_middleware_edges():
    from middleware import sanitize_sql, is_sql_safe, escape_html, strip_tags, csrf_token, verify_csrf, get_security_headers, is_rate_limited, security_headers
    ...
    parts = raw.split(".")
    tampered = __import__("base64").urlsafe_b64encode(f"{parts[0]}.{parts[1]}.bad{parts[2]}".encode()).decode()
...
def test_crypto_audit_anomaly_edges():
    from crypto import encrypt_at_rest, decrypt_at_rest, hash_password, verify_password, TLS_CHECKLIST, FALLBACK_WARNING
    # roundtrip
    ...
    for _ in range(15): d.observe("k", 1.0)
    ...
    import time
    now=time.time()
    for i in range(10): d.observe("r", 1.0, ts=now - i*0.1)
...
def test_regression_bio_neural():
    ...
    # neural bce
    e3 = SimulationEngine("neural", {"layers":[2,4,1],"activation":"sigmoid","loss":"bce","seed":7})
