...
def test_state_utils(tmp_path):
    ...
    e = SimulationEngine("physics", {"n": 4, "seed": 1}); e.run(3, 0.016)
    js = to_json_str({"kind": "physics", "params": e.get_params(), "state": e.get_state()})
    assert '"physics"' in js
    p = str(tmp_path / "x.csv")
    to_csv(e.get_state(), p); assert os.path.exists(p)
...
def test_real_parquet_hdf5(tmp_path):
    pd = pytest.importorskip("pandas"); pytest.importorskip("pyarrow"); pytest.importorskip("h5py")
    from simcore.export import export_parquet, export_hdf5
    e = SimulationEngine("particles", {"n": 10, "seed": 1}); e.run(3, 0.016)
    pp, hh = str(tmp_path/"r.parquet"), str(tmp_path/"r.h5")
    export_parquet(e.get_state(), pp); export_hdf5(e.get_state(), hh)
