...
def test_api_full_flow():
    ...
    assert set(c.get("/api/simulations/kinds").json()) >= {"particles","fluids","physics","neural","bio"}
    r = c.post("/api/auth/register", json={"username":"qa_u","password":"qa_pass123"})
    assert r.status_code in (200, 400, 409)
    t = c.post("/api/auth/login", data={"username":"qa_u","password":"qa_pass123"})
    assert t.status_code == 200, t.text
    tok = t.json()["access_token"]; H = {"Authorization": f"Bearer {tok}"}
    s = c.post("/api/simulations", json={"kind":"particles","params":{"n":20,"seed":1}}, headers=H)
    assert s.status_code in (200, 201); sid = s.json()["id"]
    st = c.post(f"/api/simulations/{sid}/step", json={"dt":0.016,"steps":5}, headers=H)
...
def test_ws_broadcast():
    ...
    tok = c.post("/api/auth/login", data={"username":"ws_u","password":"ws_pass123"}).json()["access_token"]
    sid = c.post("/api/simulations", json={"kind":"particles","params":{"n":10}}, headers={"Authorization":f"Bearer {tok}"}).json()["id"]
    with c.websocket_connect(f"/ws/simulations/{sid}?token={tok}") as w:
        w.send_json({"dt":0.016,"steps":2}); m = w.receive_json()
...
def test_security_units():
    ...
    t = csrf_token(); assert verify_csrf(t)
    from anomaly import AnomalyDetector
    d = AnomalyDetector(window=10, z_threshold=2.0)
    for _ in range(10): d.observe("lat", 1.0)
...
def test_e2e_engine_to_report(tmp_path):
    ...
    e = SimulationEngine("bio", {"model":"sir"}); e.run(30, 0.1)
    st = e.get_state()
    hist = st.get("history", [])
    out = generate_report({"simulation":{"id":1,"type":"sir","name":"E2E"},"params":e.get_params(),"series":{"I":[h.get("i",0) for h in hist]}}, str(tmp_path/"rep"))
