...
def test_particles_runs_and_bounces():
    ...
    p0 = e.get_params(); e.set_params({**p0, "gravity": 1.0}); assert e.get_params()["gravity"] == 1.0
...
def test_neural_xor_learns():
    import numpy as np
    e = SimulationEngine("neural", {"layers": [2, 8, 1], "activation": "tanh", "seed": 1})
    X = np.array([[0,0],[0,1],[1,0],[1,1]], float); y = np.array([[0],[1],[1],[0]], float)
    sim = e._sim; sim.train(X, y, epochs=400, lr=0.05)
...
def test_state_roundtrip(tmp_path):
    ...
    e.run(5, 0.016)
    p = str(tmp_path / "s.json"); save_json({"kind":"particles","params":e.get_params(),"state":e.get_state()}, p)
    d = load_json(p); assert d["kind"] == "particles"

def test_export_all_formats(tmp_path):
    from simcore.export import export_csv, export_json, export_parquet, export_hdf5
    e = SimulationEngine("particles", {"n": 10, "seed": 1}); e.run(5, 0.016)
    st = e.get_state()
    export_csv(st, str(tmp_path/"a.csv")); assert os.path.exists(str(tmp_path/"a.csv"))
    export_json(st, str(tmp_path/"a.json")); assert os.path.exists(str(tmp_path/"a.json"))
